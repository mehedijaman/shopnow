<template>
  <div class="space-y-4">
    <div v-for="attr in attributes" :key="attr.id" class="space-y-2">
      <label class="block text-sm font-medium text-gray-700">{{ attr.name }}</label>
      ...
      <div v-else-if="attr.input_type === 'image'" class="flex flex-wrap gap-2">
        <button
          ...
          @click="selectValue(attr.id, val.id)"
        >
          <span v-if="!val.swatch" class="flex h-full w-full items-center justify-center text-xs text-gray-400">{{ val.value.charAt(0) }}</span>
        </button>
      </div>
    ...
    <div v-if="variation">
      <div class="flex items-center gap-3">
        ...
        <span v-if="variation.quantity <= 0" class="rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-medium text-red-600">Out of Stock</span>
        <span v-else-if="variation.quantity < 10" class="rounded-full bg-orange-50 px-2.5 py-0.5 text-xs font-medium text-orange-600">Only {{ variation.quantity }} left</span>
