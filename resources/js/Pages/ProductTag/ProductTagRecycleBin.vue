<template>
    <Head title="Product Tags Recycle Bin"></Head>
    <AppSectionHeader title="Product Tags — Recycle Bin" :bread-crumb="breadCrumb">
        <template #right>
            <div class="flex gap-2">
                <AppButton class="btn btn-secondary" @click="$inertia.visit(route('productTag.index'))">
                    <i class="ri-arrow-left-line mr-1"></i> Back
                </AppButton>
                <AppButton v-if="can('product-tag-recycle-bin-restore') && tags.data.length"
                    class="btn btn-secondary" @click="restoreAll">
                    <i class="ri-arrow-go-back-line mr-1"></i> Restore All
                </AppButton>
                <AppButton v-if="can('product-tag-recycle-bin-delete') && tags.data.length"
            ...
            <tbody>
                <AppDataTableRow v-for="(item, index) in tags.data" :key="item.id">
                    ...
                    <AppDataTableData class="w-40 text-right">
                        <AppTooltip v-if="can('product-tag-recycle-bin-restore')" text="Restore" class="mr-3">
                            <AppButton class="btn btn-icon btn-primary"
                                @click="router.get(route('productTag.recycleBin.restore', item.id))">
                                <i class="ri-arrow-go-back-line"></i>
                            </AppButton>
                        </AppTooltip>
                        <AppTooltip v-if="can('product-tag-recycle-bin-delete')" text="Delete Permanently">
                            <AppButton class="btn btn-icon btn-destructive"
                                @click="confirmDelete(route('productTag.recycleBin.destroyForce', item.id))">
...
const restoreAll = () => router.get(route('productTag.recycleBin.restoreAll'))
const emptyBin = () => confirmDialogRef.value.openModal(route('productTag.recycleBin.empty'))
